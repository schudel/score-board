...
namespace ScoreBoard.Skills.Elo
{
    ...
    {
        private readonly TwoPlayerEloCalculator twoPlayerEloCalc;
                                          ...
                                          PairwiseComparison weakToStrongComparison)
        {
            ...
            var duelOutcomes = twoPlayerEloCalc.CalculateNewRatings(gameInfo, 
                                                                     Teams.Concat(
                                                                       new Team<TPlayer>(player1, player1Rating),
                                                                       new Team<TPlayer>(player2, player2Rating)),
                                                                        weakToStrongComparison == PairwiseComparison.Win ? new[] { 1, 2 }
                                                                      : weakToStrongComparison == PairwiseComparison.Lose ? new[] { 2, 1 }
                                                                      : new[] { 1, 1});
        ...
        private static void UpdateDuelInfo<TPlayer>(IDictionary<TPlayer, IDictionary<TPlayer, double>> duels, 
                                                    ...
                                                    TPlayer opponent )
        {
            if(!duels.TryGetValue(self, out var selfToOpponentDuelDeltas))
            {
