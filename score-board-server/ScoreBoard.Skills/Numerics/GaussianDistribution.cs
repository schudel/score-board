...
namespace ScoreBoard.Skills.Numerics
{
    public class GaussianDistribution
    {
        ...
        public double NormalizationConstant => 1.0/(Math.Sqrt(2*Math.PI)*StandardDeviation);
        ...
        public static GaussianDistribution FromPrecisionMean(double precisionMean, double precision)
        {
            ...
            {
                Precision = precision, PrecisionMean = precisionMean, Variance = 1.0 / precision
            };
        ...
        {
            if (Math.Abs(left.Precision) < Epsilon || Math.Abs(right.Precision) < Epsilon)
            {
                return 0;
            }
        ...
        {
            if (Math.Abs(numerator.Precision) < Epsilon || Math.Abs(denominator.Precision) < Epsilon)
            {
                return 0;
            }
